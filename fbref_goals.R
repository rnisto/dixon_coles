...
# need to build in pause for longer data pulls. 
# eng_2025 <- fb_match_results(country = "ENG", gender = "M", season_end_year = seq(2000,2025), tier = c("1st"))
# saveRDS(eng_2025, "./data/epl00-25.rds")
# eng_2025 <- read_rds("./data/epl00-25.rds")
...
for (i in 1:length(dates)){
  ...
  calcs <- x %>%
    mutate(
      ...
      home_chg = (HomeGoals - home_pG) * k,
      away_chg = (AwayGoals - away_pG) * k,
...
ggplot(data = filter(elo, date == date_i), aes(x = team, y = xAt)) +
  geom_col() +
  coord_flip()
...
saveRDS(elo,"./outputs/epl_0025.rds")
